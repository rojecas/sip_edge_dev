...
- [x] T1 — Crear estructura de directorios: `src/tools/`, `data/readings/`.
      Cubre: R1, R11.

- [x] T2 — Implementar `scripts/generate_readings.py` con las 5 funciones generadoras
      (una por dataset A–E) usando `random` de stdlib. Cada dataset debe tener exactamente
      50 filas con las 7 columnas especificadas.
      Cubre: R15, R16.

- [x] T3 — Agregar `if __name__ == "__main__":` en `scripts/generate_readings.py` con
      argparse para `--output-dir` (default `data/readings/`). Al ejecutarse sin args,
      genera los 5 archivos `dataset_A.csv` a `dataset_E.csv`.
      Cubre: R16.

- [x] T4 — Ejecutar `python scripts/generate_readings.py` para pre-generar los 5 datasets
...
- [x] T5 — Implementar función `load_dataset(data_dir: str, dataset_id: str) -> list[dict]`
...
- [x] T6 — Implementar función `_current_reading(pointer: tuple[int,int], dataset: list[dict],
...
- [x] T7 — Implementar función `_build_extended_response(reading: dict, address: str) -> str`
      que construya la cadena de respuesta extendida: `01ST,1,<peso>,PT 0.0,0,kg\r\n`.
      Cubre: R2.

- [x] T8 — Implementar función `_build_ok_response() -> str` que devuelva `OK\r\n`.
      Cubre: R3, R4.

- [x] T9 — Implementar la clase o funciones `_simulate_stability(status: str) -> None`
...
- [x] T10 — Implementar el parseador de comandos seriales: dada una línea del puerto,
...
- [x] T11 — Implementar el bucle principal del REPL con `msvcrt` para lectura de teclado
      sin Enter. Procesar las teclas: `n`, `p`, `w`, `g`, `s`, `q`, `Espacio`, `d`.
      Cubre: R7, R8, R9.

- [x] T12 — Implementar el manejador de tecla `n` (next): avanzar sub-paso o fila según
      la lógica del puntero. No avanzar si ya está en la última medida, último sub-paso.
      Cubre: R7.

- [x] T13 — Implementar el manejador de tecla `p` (previous): retroceder sub-paso o fila
      según la lógica del puntero. No retroceder antes de (fila=0, sub_paso=0).
      Cubre: R7, R8.

- [x] T14 — Implementar los manejadores de teclas `w` (override peso, solicitar valor
...
- [x] T15 — Implementar la función `show_status()` que muestre: dataset activo, fila actual/
      total filas, sub-paso nombre/índice, peso actual, status del CSV, si hay override activo,
      puerto y baudrate.
      Cubre: R7.

- [x] T16 — Implementar el bucle principal `main()` con argparse (`--port`, `--baudrate`,
...
- [x] T17 — Implementar cierre limpio al recibir `q` o `Ctrl+C`: cerrar puerto serial,
      imprimir mensaje de despedida, salir con código 0.
      Cubre: R7.

- [x] T18 — Crear `docs/virtual_scale_setup.md` con instrucciones de conexión física,
...
- [x] T19 — Crear `tests/test_virtual_scale.py` con tests unitarios:
...
- [x] T20 — Verificar que `python src/tools/virtual_scale.py --help` muestra todos los
      parámetros. Verificar que `python scripts/generate_readings.py` genera los 5 CSVs.
      Cubre: R14, R16.

- [x] T21 — Verificar que el mapa de trazabilidad R→test se documenta en
