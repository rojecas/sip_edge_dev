...
## Alternativa descartada: PANTONE exactos del manual de marca

**Usar los colores PANTONE exactos especificados en `feature_list.json`:
primario `#fab900` (PANTONE 130 C) y acento `#ff0032` (PANTONE RED 032 C).**

Se descartó porque: (1) los colores PANTONE están optimizados para impresión
offset (CMYK), no para pantallas digitales (RGB/sRGB); (2) `#ff0032` como fondo
de sidebar produce fatiga visual y baja legibilidad de texto blanco sobre rojo
saturado en exposicion prolongada (kiosco industrial 8+ horas); (3) el logo
corporativo en `docs/logo-mayaguez.png`, al ser muestreado digitalmente, produce
el amarillo `#FDB814` que es más cálido y legible en pantalla que `#fab900`.
La paleta final (`#FDB814`, `#32373c`) mantiene la identidad visual reconocible
de Mayaguez optimizada para el contexto de uso real.
