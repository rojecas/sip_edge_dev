...
- [x] T1 — Crear migración `database/migrations/2026_07_14_000001_add_archived_to_sms_conversations.sql` que ALTER TABLE sms_conversations MODIFY COLUMN status para agregar 'archived' al ENUM. Cubre: R12.

- [x] T2 — Crear migración `database/migrations/2026_07_14_000002_create_sms_ai_tool_log.sql` con la tabla `sms_ai_tool_log` según R5. Cubre: R5.

- [x] T3 — Agregar modelo ORM `SmsAiToolLog` en `src/models.py` con columnas, FK e índices según R5. Agregar `'archived'` al ENUM `status` de `SmsConversation`. Cubre: R5, R12.

- [x] T4 — Agregar métodos en `src/sms_persistence.py`:
...
- [x] T5 — Crear `src/ai_multi_turn.py` con la clase `AiMultiTurnService` y los siguientes métodos:
  - `__init__(db_session_factory, persistence)`
  - `get_or_create_ai_conversation(peer_number, conversation_id=None) -> SmsConversation`
    - **IMPORTANTE:** Ver design.md §14 (Integración con Dispatcher v2). Si `conversation_id` es provisto y la conversación tiene `workflow_type='unknown'`, actualizarla a `'ai_query'` en lugar de crear una duplicada.
...
- [x] T6 — Escribir tests unitarios de `src/ai_multi_turn.py` en `tests/test_ai_multi_turn.py`:
  ...
  - `test_get_or_create_ai_conversation_upgrades_unknown` — cuando el dispatcher crea conversación con workflow_type='unknown', AiMultiTurnService la actualiza a 'ai_query' en lugar de duplicar. Cubre: R1, R7.
...
- [x] T7 — Modificar `src/agent_orchestrator.py`:
  ...
  - Implementar el nuevo flujo multiturno según §11 de design.md:
    1. Obtener/crear conversación ai_query vía AiMultiTurnService (pasando `conversation_id` recibido del dispatcher).
...
- [x] T8 — Actualizar tests de `tests/test_agent_orchestrator.py`:
...
- [x] T10 — Escribir test de integración en `tests/test_ai_multi_turn_integration.py`:
  ...
  - `test_dispatcher_unknown_conversation_upgrade` — verificar que una conversación creada por el dispatcher como 'unknown' se actualiza a 'ai_query' sin duplicar. Cubre: R1, R7.

## Fase 5 — Verificación y Regresión

- [x] T11 — Verificar que los handlers de emergency y password_reset tienen prioridad:
...
- [x] T12 — Ejecutar tests de regresión en features afectadas:
  ```bash
...
- [x] T13 — Verificar `./init.ps1` pasa con todos los bloques [OK].
  Cubre: R1-R12.
...
```markdown
## Trazabilidad
- R1 → test_handle_sms_query_multiturn_uses_conversation, test_dispatcher_unknown_conversation_upgrade, T5, T7, T9
