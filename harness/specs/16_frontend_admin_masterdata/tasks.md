...
- [x] T1 — Verificar que `AdminUsers.svelte` carga lista de usuarios:
...
- [x] T7 — Verificar paginacion en tabla de usuarios (problema conocido):
...
- [x] T8 — Verificar `AdminHaciendas.svelte`:
...
- [x] T11 — Verificar `AdminSuertes.svelte`:
...
- [x] T17 — Ejecutar `npm run build` en `frontend/`:
  - Sin errores de compilacion
  Cubre: verificacion Nivel 1.

- [x] T18 — Copiar `frontend/dist/*` a `src/static/`:
...
- [x] T20 — Verificar trazabilidad completa en `progress/impl_frontend_admin_masterdata.md`:
