...
> Cambios vs original: clarificado list_weighings manual construction,
> clarificado RS232 two-step modification, documentado empty-string handling.
...
el historial (HistoryTable), y exponer al agente AI para consultas SMS.
...
| `frontend/src/components/HistoryTable.svelte` | Agregar columna "Notas" en tabla, mostrar "—" si es NULL/vacío |
...
- **CRÍTICO:** `list_weighings()` construye `WeighingResponse` manualmente.
  ...
  (líneas 263-280 de `src/weighings.py`). Sin este cambio, el historial no
  mostrará notas aunque existan en la BD.
...
- Nueva columna "Notas" en la tabla HTML.
...
| HistoryTable | `frontend/src/components/HistoryTable.svelte` | Agregar columna Notas | No |
