...
- [ ] T1 — Crear `src/config.py` con modelos Pydantic (`ScaleConfig`, `GSMConfig`, `ReportsConfig`, `SystemConfig`) y validaciones de campo (baudrate range, formato hora HH:MM, formato teléfono internacional). Cubre: R2, R3, R9, R10.
...
- [ ] T4 — Crear `src/routers/__init__.py` y `src/routers/config.py` con endpoints:
  ...
  - `POST /api/config/test-scale` → prueba conexión serial báscula.
...
- [ ] T7 — Crear `templates/admin/config_form.html`: partial HTMX con formulario de báscula + GSM + botones Test. Maneja respuestas de validación inline. Cubre: R2, R3, R4, R5, R6.
...
- [ ] T10 — Crear `tests/test_config.py`: tests unitarios de modelos Pydantic (validación de campos, defaults, valores inválidos). Cubre: R2, R3, R6, R9, R10.
...
- [ ] T12 — Crear `tests/test_config_api.py`: tests de endpoints FastAPI con `TestClient` (GET config, PUT config válido, PUT con datos inválidos, test-scale con/sin puerto, test-gsm con/sin puerto, acceso sin auth). Cubre: R1, R4, R5, R6, R12.

---

## Trazabilidad Tasks → Requirements

| Task | R1 | R2 | R3 | R4 | R5 | R6 | R7 | R8 | R9 | R10 | R11 | R12 |
|------|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:---:|:---:|:---:|
| T1   |    | X  | X  |    |    |    |    |    | X  |  X  |     |     |
| T2   |    |    |    |    |    | X  | X  |    |    |     |  X  |     |
| T3   |    |    |    |    |    |    | X  |    |    |     |     |  X  |
