...
for f in harness/AGENTS.md harness/feature_list.json harness/progress/current.md harness/docs/architecture.md harness/docs/conventions.md harness/docs/verification.md harness/CHECKPOINTS.md; do
  if [ ! -f "$f" ]; then
...
try:
    ...
    valid = {"pending", "spec_ready", "spec-reviewed", "in_progress", "testing", "done", "blocked"}
    ...
    if len(in_progress) > 1:
        ...
        sys.exit(1)
    requires_spec = {"spec_ready", "spec-reviewed", "in_progress", "testing", "done"}
    spec_errors = []
    for f in data["features"]:
        ...
        if f.get("sdd") and f["status"] in requires_spec:
            padded_id = f"{f['id']:02d}"
            spec_dir = os.path.join("harness", "specs", f"{padded_id}_{f['name']}")
