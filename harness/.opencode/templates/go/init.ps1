...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        fail "go no esta instalado o no esta en PATH"
        exit 1
    }
} catch {
    fail "go no esta instalado o no esta en PATH"
...
if ($isInContainer) {
    ...
    if ((Get-Content "harness/docs/environment.md" -Raw) -notmatch "docker") {
        warn "environment.md no refleja el uso de Docker. Revisa la seccion 'Template Docker' y actualizalo."
    }
} else {
...
Write-Host ""
Write-Host "-- 6. Verificando formato y linter -------------------------"

if (Test-Path -LiteralPath "go.mod" -PathType Leaf) {
    $fmtDiff = & gofmt -d . 2>&1
    if ($LASTEXITCODE -eq 0 -and [string]::IsNullOrWhiteSpace($fmtDiff)) {
        ok "Formato correcto (gofmt)"
    } else {
        warn "Hay diferencias de formato. Ejecuta: gofmt -w ."
    }

    if (Get-Command "golangci-lint" -ErrorAction SilentlyContinue) {
        & golangci-lint run ./... 2>&1
        if ($LASTEXITCODE -eq 0) {
            ok "Linter sin errores"
        } else {
            fail "golangci-lint encontro problemas"
        }
    } else {
        warn "golangci-lint no instalado. Skipping."
    }
} else {
    warn "go.mod no existe todavia"
}
