...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ok "rustc -> $rustcVersion"
    } else {
        fail "rustc no esta instalado o no esta en PATH"
        exit 1
    }
} catch {
    fail "rustc no esta instalado o no esta en PATH"
...
if (Test-Path -LiteralPath "Cargo.toml" -PathType Leaf) {
    & cargo fmt --check 2>&1
    if ($LASTEXITCODE -eq 0) {
        ok "Formato correcto (cargo fmt)"
    } else {
        warn "Hay problemas de formato. Ejecuta: cargo fmt"
    }
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        fail "Clippy encontro problemas"
        $script:exitCode = 1
    }
} else {
    warn "Cargo.toml no existe todavia"
}
