...
2. Lanza **1 subagente `bug-fixer`** en modo diagnostico (Fase 1) pasandole `id` y `name`.
3. Cuando el `bug-fixer` reporta `plan_ready`:
   a. Lee `harness/progress/plan-bug-<name>.md` y presentalo al humano.
   b. **PARA.** Mensaje al humano: "Plan de bug listo en `harness/progress/plan-bug-<name>.md`. Revisalo y di **'aprobado'** para que el bug-fixer implemente, o pideme cambios."
4. Cuando el humano aprueba el plan:
   a. Vuelve a lanzar **1 subagente `bug-fixer`** en modo implementacion (Fase 2), pasandole `id` y `name`.
   b. Cuando el `bug-fixer` reporta `done`:
      i. Lanza **1 `reviewer`** con instrucciones de revision de bug (verificar `reproduction` cubierto por test, no exigir trazabilidad R<n>).
      ii. Si el reviewer aprueba → lanza **1 subagente `release-manager (register)`** pasandole `id` y `name`.
      iii. Si el reviewer rechaza → reabre con `triaged` para que el bug-fixer corrija.
5. Si el `bug-fixer` reporta `blocked` (en cualquier fase):
   a. Mantiene `blocked`.
...
|-----------------------|------------------------------------------------------------------------------|--------------------------------------------------------|
| Trivial (1 archivo)   | 1 spec-author → ⏸ → 1 implementer → 1 reviewer → 1 release-manager          | 1 bug-fixer → 1 reviewer → 1 release-manager           |
| Media (2-3 archivos)  | 1 spec-author → ⏸ → 1 implementer → 1 reviewer → 1 release-manager          | 1 bug-fixer → 1 reviewer → 1 release-manager           |
| Compleja (refactor)   | 2-3 explore → 1 spec-author → ⏸ → 1 implementer → 1 reviewer → 1 release-manager | 1 bug-fixer → 1 reviewer → 1 release-manager      |
