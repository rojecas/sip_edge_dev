# Especificación de Requisitos de Software (ERS) – Versión 1.2 
...
- **Hardware de Proceso**: Báscula Serial (conectada vía RS232/USB al EdgeBox).
...
| Serial (RS232) | Adaptador USB-Báscula | Comunicación comando-respuesta |
...
`[RF-003]` **[Must Have]** Interacción Bidireccional con Báscula (Comando-Respuesta): Protocolo activo donde el sistema:
- Envía comando específico (string/hex) al puerto serial al presionar botones de control (Tara Total, Peso Mineral, etc.).
...
> ✅ **Corrección aplicada**: Numeración secuencial corregida (RF-008 reinsertado). Eliminado salto RF-008→RF-009 original.
...
`[RF-015]` **[Must Have]** Configuración Dinámica de Puertos: Interfaz gráfica Admin para modificar rutas hardware (Báscula, Módem), baudrate, paridad.
...
Este documento **ERS v1.2** está listo para servir como base del Software Design Document (SDD). ¿Le gustaría que procedamos con el análisis de retos técnicos y stack tecnológico para definir el cronograma de desarrollo?